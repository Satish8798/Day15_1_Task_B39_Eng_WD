...
function goToLast(){
  ...
  tableBody.innerHTML=`
      <tr>
        ...
        <td>${data[data.length-1].email}</td>
      </tr>
  `
  for(let i=numbers.length-1,k=0;i>=0;i--,k++){
    numbers[k].innerText=data.length-i;
  }
}
...
numbers.forEach((num)=>{
  num.addEventListener("click",function(){
    let goToNumber=num.innerText;
    console.log(goToNumber)
    number=parseInt(goToNumber)-1;
    tableBody.innerHTML=`
        <tr>
          <td>${data[number].id}</td>
          <td>${data[number].name}</td>
...
function setNumbersNext(){
  if(numbers[0].innerText==='96'){
    return 0;
  }
  for(let i=0;i<numbers.length;i++){
   let presentNumber=parseInt(numbers[i].innerText);
   numbers[i].innerText=presentNumber+1;
  }
}

function setNumbersPrevious(){
  if(numbers[0].innerText==='1'){
    return 0;
  }
  for(let i=0;i<numbers.length;i++){
   let presentNumber=parseInt(numbers[i].innerText);
